...
def main(dimensiones):
    ...
    # Crear y mostrar el diálogo# Obsolet, this option has been removed
    ...
    for tratamiento in range(1, num_tratamientos + 1):
        ...
        messagebox.showinfo("Información", f"Seleccione las celdas según el tipo")
...
def initial_settings():
    ...
    # Row 5: 
    tk.Label(root, text="Tiempo de imagen para realizar la deteccion de los tubos (Poner -1 si la imagen del tiempo final):").grid(row=5, column=0, padx=10, pady=5, sticky="w")
    entry_time_choose_image = tk.Entry(root)
...
def get_crop_coordinates(image_ndarray, pixel_x_1, pixel_y_1, pixel_x_2, pixel_y_2):
    ...
    if len(image_ndarray.shape)==5:
        rgb_array = image_ndarray[-1, 1, :, :, :]
    elif len(image_ndarray.shape)==4:
        #print('que es lo que pasa ', image_ndarray.shape)
        rgb_array = image_ndarray[-1, :, :, :]
        rgb_array = rgb_array.transpose(1,2,0)
        #print('esto pasa ', rgb_array.shape)
